...
public class FettlerActivity extends AppCompatActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_fettler);
...
//动态权限申请
        ...
        if (ContextCompat.checkSelfPermission(FettlerActivity.this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
            permissionList.add(Manifest.permission.CAMERA);
        }
        if (!permissionList.isEmpty()) {
            String [] permissions = permissionList.toArray(new String[permissionList.size()]);
            ActivityCompat.requestPermissions(FettlerActivity.this, permissions, 1);
        }
        ...
        scanButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent intent = new Intent(FettlerActivity.this, CaptureActivity.class);
...
//    }
    ...
    public boolean onMarkerClick(final Marker marker) {
        ...
        Toast.makeText(this, "所有显示点均需维修", Toast.LENGTH_LONG).show();
        return true;
    }
...
//    };
    ...
    Handler handler2=new Handler(){
        public void handleMessage(Message msg){
            ...
            String jsonResult= msg.obj.toString();
            Log.e("验证状态jsonResult",jsonResult);
            if(jsonResult.equals("0"))
            {
                Toast.makeText(getApplicationContext(), "此按摩椅不需要为护", Toast.LENGTH_SHORT).show();
            }
            else{
                Log.e("查询结果jsonResult",jsonResult);
                Log.e("查询结果msg.obj.toString()",msg.obj.toString());
                ...
                outLoop: for (MassageChair massageChair:massageChairs){
                    if(chairNum==Integer.parseInt(result)){
                        //开始使用
                        //开启线程操作数据库,写入状态
                        Thread thread = new Thread(new Runnable() {
                            @Override
...
//    }
    ...
    public String Select1() {
        ...
        String sql = "SELECT [MC_state] AS MC_state FROM [MassageChair] where [number]="+"'"+result+"'";
        String jsonResult = null;
        try {
            // sh.ExecuteQuery()，参数1：查询语句，参数2：查询用到的变量，用于本案例不需要参数，所以用空白的new
            // ArrayList<Object>()
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_SCAN && resultCode == RESULT_OK) {
            if (data != null) {
                ...
                numberEditText.setText(content);//("扫描结果为：" + content);
                result=content;//按摩椅id


                chairNum = Integer.parseInt(result);
//先查询编号是否故障(有必要?),然后,设置状态
                //开启线程操作数据库,查询编号
                Thread thread = new Thread(new Runnable() {
                    @Override
                    public void run() {
